...
AFFIRMATIVE_REGEX = re.compile(
    "|".join(AFFIRMATIVE_PATTERNS), re.IGNORECASE | re.UNICODE
)


async def clear_waiting_state(user_id, delay=30):
    await asyncio.sleep(delay)
    if user_id in pending_punishments:
        del pending_punishments[user_id]
...
def load_bad_words_database():
    ...
    if os.path.exists("bad_words.json"):
        try:
            ...
            print(
                f"✅ تم تحميل {len(BAD_WORDS_SET)} كلمة ممنوعة في الذاكرة بنجاح!"
            )
...
class ScriptDetailButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        key_status = (
            "نعم" if self.script_data.get("key", False) else "لا (بدون كي)"
        )
...
class MultiRoleSelectView(discord.ui.View):
    ...
    def __init__(self, guild: discord.Guild):
        ...
        for index, role_chunk in enumerate(chunks):
            ...
            self.add_item(
                RoleSelect(roles=role_chunk, placeholder=placeholder_text)
            )
...
@clear_group.command(
    name="role",
    description=(
        "عرض جميع رتب السيرفر في قوائم منسدلة وااختيار رتبة لسحبها من الجميع"
    ),
)
...
async def on_message(message: discord.Message):
    ...
    if user_id in pending_punishments:
        ...
        if AFFIRMATIVE_REGEX.search(cleaned_content) or cleaned_content in [
            ...
            try:
                ...
                # ب. إعطاء رتبة العقوبة
                punishment_role = message.guild.get_role(PUNISHMENT_ROLE_ID)
                if punishment_role:
                    await message.author.add_roles(punishment_role)

                # ج. إعطاء تايم أوت 5 دقائق
                duration = timedelta(minutes=5)
                await message.author.timeout(
                    duration, reason="تحدي الإدارة بعد الشتم"
                )

                await message.channel.send(
                    f"⛔ {message.author.mention} تم تنزيل رتبك وإعطاؤك رتبة"
                    " العقوبة مع تايم أوت 5 دقائق!"
                )

                # د. الانتظار 5 دقائق ثم إرجاع الرتب
                await asyncio.sleep(300)

                if punishment_role and punishment_role in message.author.roles:
                    await message.author.remove_roles(punishment_role)

                await message.author.add_roles(*saved_roles)
                await message.channel.send(
                    f"✅ انتهت العقوبة وتمت إعادة كافة الرتب القديمة لـ"
            ...
            data = pending_punishments.pop(user_id, None)
    ...
    if is_admin and is_profane(content):
        ...
        task = asyncio.create_task(clear_waiting_state(user_id, delay=30))
        pending_punishments[user_id] = {"roles": original_roles, "task": task}

        await message.channel.send(
            f"بدك انزل رتبتك واجلدك؟ {message.author.mention}"
        )
        return
    ...
    if content == "#فتح":
        ...
        embed = discord.Embed(
            description="🔓 **تم فتح الشات بنجاح.**",
            color=discord.Color.green(),
        )
        await message.reply(embed=embed)
        return
    ...
    if content.startswith("اص"):
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    f"🤐 **تم إعطاء {target.mention} تايم أوت لمدة"
                    f" `{duration_str}`.**"
                ),
    ...
    if content.startswith("تكلم") or content.startswith("تكلموا"):
        ...
        try:
            await target.timeout(
                None, reason=f"فك الإسكات بواسطة {message.author}"
            )
            embed = discord.Embed(
    ...
    if content.startswith("ارجاع"):
        ...
        try:
            await message.guild.unban(
                target, reason=f"فك الحظر بواسطة {message.author}"
            )
            embed = discord.Embed(
    ...
    if content.startswith("تفضل"):
        ...
        if not role:
            await message.reply(
                "❌ **خطأ:** لم يتم العثور على الرتبة بالـ ID المحدد!"
            )
            return
        try:
            ...
            embed = discord.Embed(
                description=(
                    f"🎭 **تم إعطاء {target.mention} رتبة {role.mention}"
                    " بنجاح.**"
                ),
                color=discord.Color.blue(),
            )
    ...
    if content == "شيل":
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    f"🧹 **تم إزالة رتبة {role.mention} بنجاح من `{count}`"
                    " عضو.**"
                ),
                color=discord.Color.green(),
            )
